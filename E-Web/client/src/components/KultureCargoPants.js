...
const KultureCargoPantsCollections = ( ) => {
  const dispatch = useDispatch(); // Get the dispatch function from Redux


  const filterItemsByCategory = (items) => {
    // Use the filter method to get items with category "KultureJackets"
    const kultureJackets = items.filter(item => item.attributes.category === "Kulture Cargo Pants");
  ...
  return (
    <section className="container py-5">
      <h2 className="mb-4">Kulture Cargo Pants Collections </h2>
...
export default KultureCargoPantsCollections;
