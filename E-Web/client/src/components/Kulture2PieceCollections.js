...
const Kulture2PieceCollections = ( ) => {
  const dispatch = useDispatch(); // Get the dispatch function from Redux


  const filterItemsByCategory = (items) => {
    // Use the filter method to get items with category "KultureJackets"
    const kultureJackets = items.filter(item => item.attributes.category === "Kulture 2 Piece");
  ...
  return (
    <section className="container py-5">
      <h2 className="mb-4">Kulture 2 Piece Collections </h2>
        ...
        {kultureJackets.map((product) => (
          <div className="col-md-4 mb-4" key={product.id}>
            ...
            src={`http://localhost:1337${product.attributes.image.data.attributes.formats.small.url}`}  // Adjust the URL according to your Strapi setup src={product.image}
                alt={product.name}
            className="custom-card-img" // Apply the custom CSS class
...
export default Kulture2PieceCollections;
